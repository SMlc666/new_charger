...
Log logger("/data/adb/bypass_charge.log", LogLevel::INFO);
Config config_tool;
...
int main()
{
    ...
    while (true)
    {
        ...
        else if (Status != BatteryStatus::Charging && Info.FastCharge_Status)
        {
            
            Info.FastCharge_Status = false;
            logger.write(LogLevel::INFO, "不处于充电状态,自动关闭闲时快充");
        }
        else if (!Have_USB && Info.Bypass_Status) {
            Info.Event = Bypass_Event::None;
            Info.Bypass_Status = false;
            logger.write(LogLevel::INFO, "没有检测到USB插入,自动关闭旁路供电");
        }
    }
    return 0;
}
